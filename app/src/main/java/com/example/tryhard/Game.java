...
public class Game extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // Set colors for the ball, paddle, and text
        ...
        if (ball.getY() + ball.getRadius() >= paddle.getPaddleY1()
            ...
            // Ball missed the paddle, life lost
            life--;
            if (life == 0) {
                ...
                intent.putExtra("points", hitCounter);  // Send score if needed
                context.startActivity(intent);
                ((Activity) context).finish();  // Finish current activity
            }
        ...
        canvas.drawRect(paddle.getPaddleX1(), paddle.getPaddleY1(),
                paddle.getPaddleX1() + paddle.getWidth(), paddle.getPaddleY1() + paddle.getHeight(), paint2);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_MOVE) {
            // Move the paddle with the touch event
